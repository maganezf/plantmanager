...
export async function savePlant(plant: IPlantProps): Promise<void> {
  try {
    ...
    const seconds = Math.abs(
      Math.ceil((DateNow.getTime() - nextTime.getTime()) / 1000)
    );
  ...
  try {
    ...
    const plantsSorted = Object.keys(plants)
      ...
      .sort((firstDate, secondDate) =>
        Math.floor(
          new Date(firstDate.dateTimeNotification).getTime() / 1000 -
            Math.floor(
              new Date(secondDate.dateTimeNotification).getTime() / 1000
            )
        )
      );
